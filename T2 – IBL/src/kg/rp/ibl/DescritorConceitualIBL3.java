...
public class DescritorConceitualIBL3 extends DescritorConceitual
{
	...
	private boolean aceitável(Classificação<String> inst)
	{
		RegistroInstância regInst = regsInsts.get(inst);
		RegistroClasse regClasse = regsClasses.get(inst.classe());
		
		return regInst.precA[0] > regClasse.freqA[1];
	}
	
	private void atualizarInstância(Classificação<String> inst, String classeReal)
	{
		RegistroInstância regInst = regsInsts.get(inst);
		RegistroClasse regClasse = regsClasses.get(inst.classe());
		
		// Atualiza blablabla
		if (inst.classe().equals(classeReal))
			regInst.acertos++;
		else
			regInst.erros++;
		regInst.atualizar();
	...
	private void criarRegistro(Classificação<String> inst)
	{
		if (!regsInsts.containsKey(inst))
			regsInsts.put(inst, new RegistroInstância());
		if (!regsClasses.containsKey(inst.classe()))
			regsClasses.put(inst.classe(), new RegistroClasse());
	}
